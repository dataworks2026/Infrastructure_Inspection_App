...
export default function CvAccuracyWidget() {
    ...
    return (
      <div className="interactive-card bg-white rounded-2xl shadow-sm overflow-hidden flex flex-col"
        ...
        <div className="flex flex-col items-center justify-center py-8 gap-2 flex-1">
          <Target size={26} style={{ color: '#C8E6D4' }} />
  ...
  const segments: { key: string; count: number; color: string }[] = [
    ...
    { key: 'Modified', count: overall.modified, color: '#E6A817' },
  ...
  const mainColor = accuracyColor(overall.avg_accuracy_pct);
    ...
    <div className="interactive-card bg-white rounded-2xl shadow-sm overflow-hidden flex flex-col"
      ...
      <div className="px-4 py-3 flex flex-col gap-3">
        ...
        <div className="flex items-end gap-3">
          <p className="text-[34px] font-black leading-none tabular-nums" style={{ color: mainColor }}>
            {overall.avg_accuracy_pct.toFixed(1)}%
          </p>
          <p className="text-[11px] font-semibold pb-0.5" style={{ color: '#6B9A87' }}>
            ...
            {' · '}{overall.total_cv_detections} CV detections
          </p>
        </div>

        {/* Stacked accepted / modified / rejected bar */}
        <div>
              ...
              <div key={key} className="flex items-center gap-2">
                ...
                <div className="flex-1 h-[5px] rounded-full overflow-hidden" style={{ background: '#EDF6F0' }}>
                  <div className="h-full rounded-full"
                    style={{ width: `${Math.min(Math.max(dt.pct, 0), 100)}%`, background: accuracyColor(dt.pct) }} />
                </div>
                <span className="text-[10px] font-black tabular-nums w-10 text-right"
                  style={{ color: accuracyColor(dt.pct) }}>
                  {dt.pct.toFixed(0)}%
                </span>
                <span className="text-[9px] font-semibold tabular-nums w-8 text-right" style={{ color: '#9AB8AD' }}>
                  {dt.cv} CV
                </span>
              </div>
            ))}
          ...
          <div className="flex flex-col gap-0.5">
            ...
            {recent.slice(0, 3).map(r => (
              <Link key={r.inspection_id}
                ...
                <span className="text-[9px] font-semibold tabular-nums flex-shrink-0" style={{ color: '#9AB8AD' }}>
                  {r.cv_detections} det
                </span>
                <span className="text-[11px] font-black tabular-nums flex-shrink-0"
                  style={{ color: accuracyColor(r.accuracy_pct) }}>
